...
import { MapPin, Search, X, CheckCircle2, AlertCircle, Loader2, ChevronDown, Navigation } from 'lucide-react';
...
export function LocationPinServiceBar({ isDesktop = false }: LocationPinServiceBarProps) {
  ...
  // Location detection & manual selection states
  ...
  // Focus management when PIN modal opens
  useEffect(() => {
    if (isPinModalOpen) {
      setTimeout(() => {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'Escape') {
        ...
        if (isCityModalOpen) setIsCityModalOpen(false);
      }
    };
    window.addEventListener('keydown', handleKeyDown);
    return () => window.removeEventListener('keydown', handleKeyDown);
  }, [isPinModalOpen, isCityModalOpen]);
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (error) => {
        ...
        if (error.code === error.PERMISSION_DENIED) {
          setLocationStatusMessage('Location permission was denied. Please select your delivery city manually.');
        } else {
          setLocationStatusMessage('Could not retrieve your location. Please select your delivery city manually.');
        }
        setIsCityModalOpen(true);
      },
  ...
  return (
    <>
      ...
      {locationStatusMessage && !isCityModalOpen && (
        <div
          role="status"
      ...
      {/* ========================================================================= */}
      {/* DESKTOP SERVICE BAR (Compact horizontal layout)                           */}
      {/* ========================================================================= */}
      {isDesktop ? (
        <div className="flex items-center justify-between text-xs py-0.5">
          <div className="flex items-center gap-3">
            {/* Option 1: Current Location with GPS & manual selector */}
        ...
        <div className="grid grid-cols-2 gap-2 pt-0.5">
          {/* Option 1: 📍 Google / Current Location */}
          <button
            type="button"
            id="mobile-current-location-btn"
            onClick={handleDetectLocation}
            className="flex items-center gap-2 px-2.5 py-1.5 rounded-xl bg-[var(--bg-subtle)] hover:bg-[var(--bg-accent)] border border-[var(--border)] text-left transition-all active:scale-[0.98] focus-visible:ring-2 focus-visible:ring-[#FF2B6D] min-w-0 cursor-pointer shadow-2xs"
            aria-label={`Current location: ${deliveryCity}. Tap to detect GPS location or change city.`}
          >
      ...
      {/* ========================================================================= */}
      {/* CITY SELECTION MODAL (Location fallback & manual picker)                  */}
      {/* ========================================================================= */}
      {isCityModalOpen && (
        <div
          role="dialog"
        ...
        >
          <div
            className="w-full max-w-sm bg-[var(--bg-surface)] border border-[var(--border)] rounded-2xl p-5 shadow-2xl space-y-4 animate-in zoom-in-95"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between">
              <div>
                <h3 id="city-modal-title" className="text-base font-bold text-[var(--text-main)]">
                  Select Delivery City
                </h3>
                <p className="text-xs text-[var(--text-muted)] mt-0.5">
                  Currently serving Gurugram &amp; Delhi NCR
                </p>
              </div>
              <button
                type="button"
                onClick={() => setIsCityModalOpen(false)}
                className="p-1.5 rounded-full hover:bg-[var(--bg-subtle)] text-[var(--text-muted)] hover:text-[var(--text-main)] cursor-pointer"
            ...
            {/* GPS Auto-Detect CTA inside modal */}
            <button
              type="button"
              onClick={() => {
              ...
              disabled={isDetecting}
              className="w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl bg-[#FF2B6D]/10 hover:bg-[#FF2B6D]/20 text-[#FF2B6D] font-bold text-xs border border-[#FF2B6D]/30 transition-all cursor-pointer"
            >
              {isDetecting ? (
            ...
            <div className="space-y-1.5 pt-1">
              <div className="text-[11px] font-bold text-[var(--text-subtle)] uppercase tracking-wider px-1">
      ...
      {isPinModalOpen && (
        ...
        >
          ...
          >
            ...
            <div className="flex items-center justify-between">
              <div>
                <h3 id="pin-modal-title" className="text-base font-bold text-[var(--text-main)]">
              ...
              <button
                ...
                onClick={() => setIsPinModalOpen(false)}
                className="p-1.5 rounded-full hover:bg-[var(--bg-subtle)] text-[var(--text-muted)] hover:text-[var(--text-main)] cursor-pointer"
                aria-label="Close delivery availability check"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <form onSubmit={handleCheckPin} className="space-y-3">
              <div>
                <label htmlFor="pin-input-field" className="sr-only">
                  6-digit PIN code
                </label>
                <div className="relative">
                  <input
                    ...
                    className="w-full px-3.5 py-2.5 rounded-xl bg-[var(--bg-subtle)] border border-[var(--border)] text-sm text-[var(--text-main)] placeholder:text-[var(--text-subtle)] focus:outline-none focus:ring-2 focus:ring-[#FF2B6D] transition-all font-mono tracking-wider"
                  />
                  {pinInput && (
                    <button
                      type="button"
